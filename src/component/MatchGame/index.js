...
class MatchGame extends Component {
  ...
  stateChange = () => {
    this.setState(prev => ({isGameStarted: !prev.isGameStarted}))
    this.setState({topScore: 0, time: 60, displayedImage: imagesList[0]})
    this.componentDidMount()
  }

  imageTab = image => {
    const {displayedImage} = this.state
    const {id} = displayedImage
  ...
  winOrLoosCard = () => {
    ...
    return <WinOrLoose score={topScore} playAgainGame={this.stateChange} />
  }
  ...
  renderOfImagesList = () => {
    const {displayedImage, activeTabId} = this.state
    const {imageUrl} = displayedImage
